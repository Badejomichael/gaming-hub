import { sportsAndRacingGameNews } from "../data/games-data.js";
...
async function getsportsAndRacingGames() {
  ...
  if (sportsAndRacingGames) {
    ...
    try {
      const response = await fetch(`https://api.rawg.io/api/games?genres=sports,racing&page_size=100&ordering=-rating&key=${apiKey}`);
    ...
    sportsAndRacingGames += `
        <div class="card" style="width: 20rem">
          ...
          <div class="card-body">
            ...
            <p class="card-text">
              A highly rated Sport & Racing with immersive gameplay and great storytelling.
            </p>
          </div>
...
async function getRandomSportsAndRacingGame() {
  ...
  if (cachedSportsAndRacingGame) {
    ...
    try {
      const response = await fetch(`https://api.rawg.io/api/games?genres=sports&page_size=100&ordering=-rating&key=${apiKey}`);
      const data = await response.json();
      ...
      // Save to localStorage
      localStorage.setItem("featuredSportsAndRacingGame", JSON.stringify(featuredSportsAndRacingGame));
...
async function searchGames(query) {
  ...
  let storedSportsGames = JSON.parse(localStorage.getItem("sports-and-racing-games"));

  let filteredSportsGames = storedSportsGames.filter(game =>
    game.name.toLowerCase().includes(newSearchInput)
  );

  if (filteredSportsGames.length > 0) {
    document.querySelector('.js-search-result-header').classList.add('positive-result-header');
    displaySearchResults(filteredSportsGames);
  ...
  // If no matches in localStorage, fetch from Rawg API
    ...
    try {
      const response = await fetch(
        `https://api.rawg.io/api/games?key=${apiKey}&search=${encodeURIComponent(query.value)}&genres=sports&page_size=20`
      );
      const data = await response.json();
      
      if (data.results && data.results.length > 0) {
        const searchedGame = data.results.filter(game => 
          game.genres && game.genres.some(genre => genre.name.toLowerCase() === "sports")
        )
